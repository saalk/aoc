...
public interface AdventOfCode {
    ...
    static String lastXChars(String v, int x) {
        //This method will return the x amount of characters from the end.
        return v.length() <= x ? v : v.substring(v.length() - x);
    }
    static <T, U> List<U> convertObjectsList(List<T> listOfObjects,
                                             Function<T, U> function) {
        // Function can be eg
        // str -> str.length() => all Strings are to their length
        // String::toUpperCase => all Strings are to upper
        // Integer::parseInt => all Strings are parsed to Integer
        // number -> number * 3 => all Integers are multiplied by 3
    ...
    static Map<String, String> getNameValuesForString(String stringToSplit, String separator) {
        // regex \s*,\s* considers white-space as part of the separator, hence trimming the entries
        // for: "name peter, add mumbai, md v ,refNo "
        if (isBlankString(separator)) separator = " ";
        String finalSeparator = separator;
        return Pattern.compile("\\s*,\\s*")
                .splitAsStream(stringToSplit.trim())
                .map(s -> s.split(finalSeparator, 2))
                .collect(toMap(a -> a[0], a -> a.length > 1 ? a[1] : ""));
    }
    static Map<String, Integer> getNameValuesForInteger(String stringToSplit, String separator) {
        // regex \s*,\s* considers white-space as part of the separator, hence trimming the entries
    ...
    default HashMap<Integer, String> getPositionValuesFromStringList(ArrayList<String> arrayList) {
